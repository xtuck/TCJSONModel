...
@implementation NSObject (TCJSONModel)
...
+(void)load {
    ...
    dispatch_once(&once, ^{
        ...
        // which are common for ALL TCJSONModel subclasses
...
+ (instancetype)tc_modelFromKeyValues:(id)keyValues error:(NSError **)err {
    //check for nil input
    if (!keyValues) {
        if (err) *err = [TCJSONModelError errorInputIsNil];
        return nil;
    }
    ...
    if (!dic) {
        if (err) *err = [TCJSONModelError errorBadJSON];
        return nil;
    }
...
-(id)initWithDictionaryTC:(NSDictionary*)dict error:(NSError**)err {
    ...
    if (!self) {
        ...
        if (err) *err = [TCJSONModelError errorModelIsInvalid];
        return nil;
    }
...
-(BOOL)__tcImportDictionary:(NSDictionary*)dict validation:(BOOL)validation error:(NSError**)err {
    ...
    for (TCJSONModelClassProperty* property in properties) {
        ...
        if (isTCNull(jsonValue)) {
            ...
            if (err) {
                ...
                TCJSONModelError* dataErr = [TCJSONModelError errorInvalidDataWithMessage:msg];
                ...
                if (
                    ...
                    if (foundCustomTransformer) {
                        ...
                        if (err) {
                            ...
                            TCJSONModelError* dataErr = [TCJSONModelError errorInvalidDataWithTypeMismatch:msg];
                            *err = [dataErr errorByPrependingKeyPathComponent:property.name];
                        }
                        return NO;
                    }
...
-(void)__tcInspectProperties {
    ...
    while (class != [NSObject class]) {
        ...
        for (unsigned int i = 0; i < propertyCount; i++) {
            ...
            TCJSONModelClassProperty* p = [[TCJSONModelClassProperty alloc] init];
...
-(id)__tcTransform:(id)value forProperty:(TCJSONModelClassProperty*)property error:(NSError**)err
{
        ...
        if ([property.type isSubclassOfClass:[NSDictionary class]]) {
            ...
            if(![[value class] isSubclassOfClass:[NSDictionary class]])
            {
                if(err != nil)
                {
                    ...
                    TCJSONModelError* typeErr = [TCJSONModelError errorInvalidDataWithTypeMismatch:mismatch];
...
-(id)__tcReverseTransform:(id)value forProperty:(TCJSONModelClassProperty*)property
{
    Class protocolClass = NSClassFromString(property.protocol);
...
- (BOOL)__tcCustomSetValue:(id <NSObject>)value forProperty:(TCJSONModelClassProperty *)property
{
...
- (BOOL)__tcCustomGetValue:(id *)value forProperty:(TCJSONModelClassProperty *)property
{
...
- (NSDictionary *)tc_toDictionaryWithKeys:(NSArray <NSString *> *)propertyNames
{
    ...
    for (TCJSONModelClassProperty* p in properties) {
...
+ (NSMutableDictionary *)tc_dictionaryOfModelsFromDictionary:(NSDictionary *)dictionary error:(NSError **)err
{
    ...
    for (NSString *key in dictionary.allKeys)
    {
        ...
        {
            if (err) {
                *err = [TCJSONModelError errorInvalidDataWithTypeMismatch:@"Only dictionaries and arrays are supported"];
            }
            return nil;
        }
    }
